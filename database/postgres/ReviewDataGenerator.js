...
let commentGenerator = (foodType, starRating) => {

  let food;
  let randomFood1;
  if (foodType === 'italian') {
    food = italianFoods;
    randomFood1 = food[Math.floor(Math.random() * food.length)]
...
let generateFakeData = (num) => {
  let fakeData = '';
  let restaurantID = Math.floor(Math.random() * 10000) + 1;
  ...
  let foodType;
  let food;
  if (restaurantID >= 1 && restaurantID <= 2000) {
    foodType = 'korean';
    food = koreanFoods;
  }
  else if (restaurantID >= 2001 && restaurantID <= 4000) {
    foodType = 'mexican';
    food = mexicanFoods;
  }
  else if (restaurantID >= 4001 && restaurantID <= 6000) {
    foodType = 'chinese';
    food = chineseFoods;
  }
  else if (restaurantID >= 6001 && restaurantID <= 8000) {
    foodType = 'italian';
    food = italianFoods;
  }
  else if (restaurantID >= 8001 && restaurantID <= 10000) {
    foodType = 'burger';
    food = burgerFoods;
  }
  ...
  instance.restaurantRating = starRating;
  instance.text = comments;
  instance.cuisine = foodType;
  instance.date = date;

  fakeData += restaurantID + ',' + instance.restaurantRating + ',' + '"' + instance.text + '"' + ',' + instance.cuisine + ',' + '"' + date + '"' + '\n'
